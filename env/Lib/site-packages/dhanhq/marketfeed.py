...
class DhanFeed:
    ...
    async def disconnect(self):
        ...
        if self.ws:
            if self.version == "v2":
                ...
                header_message = self.create_header(
                    feed_request_code=12, message_length=83, client_id=self.client_id
                )
    ...
    async def authorize(self):
        ...
        try:
            ...
            dhan_auth = b"\0" * 50
            header = struct.pack(
                "<bH30s50s", feed_request_code, message_length, client_id, dhan_auth
            )
    ...
    def validate_and_process_tuples(self, tuples_list, batch_size=100):
        ...
        if len(tuple_lengths) > 1:
            raise ValueError(
                "All tuples must be of the same size, either all 2 or all 3."
            )
        ...
        for tup in processed_tuples:
            ...
            if self.version == "v1" and type_ not in [15, 17, 19]:
                raise ValueError(
                    f"Invalid request mode for v1. Only Ticker, Quote, and Depth packet are allowed."
                )
            if self.version == "v2" and type_ not in [15, 17, 21]:
                raise ValueError(
                    f"Invalid request mode for v2. Only Ticker, Quote, and Full packet are allowed."
                )
        ...
        for type_ in [15, 17, 19, 21]:
            type_batches = [
                batches[type_][i : i + batch_size]
                for i in range(0, len(batches[type_]), batch_size)
            ]
    ...
    async def subscribe_instruments(self):
        ...
        if self.version == "v1":
            ...
            new_instrument_list = self.validate_and_process_tuples(
                self.instruments, group_size
            )
                ...
                for instrument_type in new_instrument_list:
                    if new_instrument_list[instrument_type] != []:
                        for instrument_group in new_instrument_list[instrument_type]:
                            ...
                            subscription_packet = self.create_subscription_packet(
                                instrument_group, self.subscription_code
                            )
    ...
    def process_market_depth(self, data):
        ...
        for i in range(5):
            ...
            current_packet = struct.unpack(
                packet_format, market_depth_binary[start_idx:end_idx]
            )
    ...
    async def on_connection_opened(self, websocket):
        ...
        await websocket.send(
            self.create_subscription_packet(self.instruments, subscribe=True)
        )
    ...
    def create_subscription_packet(self, instruments, feed_request_code):
        ...
        header = self.create_header(
            feed_request_code=feed_request_code,
        ...
        for exchange_segment, security_id in instruments:
            instrument_info += struct.pack(
                "<B20s", exchange_segment, security_id.encode("utf-8")
            )

        instruments = [(0, "")]
        for i in range(100 - num_instruments):
            instrument_info += struct.pack(
                "<B20s", instruments[0][0], instruments[0][1].encode("utf-8")
            )
            ...
            if self.version == "v1":
                for instrument_type in new_instrument_list:
                    if new_instrument_list[instrument_type]:
                        for instrument_group in new_instrument_list[instrument_type]:
                            subscription_packet = self.create_subscription_packet(
                                instrument_group, int(instrument_type)
                            )
                    ...
                    for instrument_group in instrument_groups:
                        for i in range(0, len(instrument_group), 100):
                            ...
                            subscription_message = {
                                ...
                                "InstrumentList": [
                                    {
                                        "ExchangeSegment": self.get_exchange_segment(
                                            ex
                                        ),
                                        "SecurityId": token,
                                    }
    ...
    def unsubscribe_symbols(self, symbols):
        ...
        if self.ws and not self.ws.closed:
            ...
            group_size = 100
            instrument_list_to_unsubscribe = self.validate_and_process_tuples(
                symbols, group_size
            )
            ...
            if self.version == "v1":
                ...
                for instrument_type in instrument_list_to_unsubscribe:
                    if instrument_list_to_unsubscribe[instrument_type]:
                        for instrument_group in instrument_list_to_unsubscribe[
                            instrument_type
                        ]:
                            # Use the original instrument_type for grouping, but increment for the actual request
                            unsubscription_packet = self.create_subscription_packet(
                                instrument_group, int(instrument_type) + 1
                            )
                ...
                for (
                    instrument_type,
                    instrument_groups,
                ) in instrument_list_to_unsubscribe.items():
                    for instrument_group in instrument_groups:
                        for i in range(0, len(instrument_group), 100):
                            batch = instrument_group[i : i + 100]
