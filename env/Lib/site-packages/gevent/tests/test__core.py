...
class WatcherTestMixin(object):
    ...
    def test_events_conversion(self):
        self.assertEqual(
            self.core._events_to_str(
                self.core.READ | self.core.WRITE
    ...
    def test_EVENTS(self):
        self.assertEqual(
            str(self.core.EVENTS), "gevent.core.EVENTS"  # pylint: disable=no-member
        )
        self.assertEqual(
            repr(self.core.EVENTS), "gevent.core.EVENTS"  # pylint: disable=no-member
        )
    ...
    def test_io(self):
        ...
        if not greentest.WIN:
            ...
            if not hasattr(self.core, "libuv"):
                ...
                self.assertEqual(
                    self.core._events_to_str(io.events),  # pylint:disable=no-member
                    "WRITE",
                )
...
class LibevTestMixin(WatcherTestMixin):
    ...
    def test_flags_conversion(self):
        ...
        self.assertEqual(
            core._flags_to_int(["kqueue", "SELECT"]),
            core.BACKEND_KQUEUE | core.BACKEND_SELECT,
        )
        self.assertEqual(
            core._flags_to_list(core.BACKEND_PORT | core.BACKEND_POLL), ["port", "poll"]
        )
