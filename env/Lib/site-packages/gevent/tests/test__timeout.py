...
class Test(greentest.TestCase):
    ...
    def assert_type_err(self, ex):
        ...
        self.assertTrue(
            "exceptions must be" in str(ex) or "exceptions must derive" in str(ex),
            str(ex),
        )
    ...
    def test_with_timeout(self):
        ...
        X = object()
        r = gevent.with_timeout(
            SHOULD_EXPIRE, gevent.sleep, SHOULD_NOT_EXPIRE, timeout_value=X
        )
        self.assertIs(r, X)
        r = gevent.with_timeout(
            SHOULD_NOT_EXPIRE, gevent.sleep, SHOULD_EXPIRE, timeout_value=X
        )
        self.assertIsNone(r)
