    ...
    while not read.endswith(postfix):
        ...
        if not result:
            raise AssertionError(
                "Connection ended before %r. Data read:\n%r" % (postfix, read)
            )
        read += result
...
class Test(greentest.TestCase):
    ...
    def _make_and_start_server(self, *args, **kwargs):
        server = backdoor.BackdoorServer(
            self._server_listen_argument(), *args, **kwargs
        )
        server.start()
        return server
    ...
    def _create_connection(self, server):
        ...
        try:
            conn.connect(
                *self._connection_arguments(server)
...
@greentest.skipUnless(hasattr(socket, "AF_UNIX"), "Needs AF_UNIX support")
class TestUnixSocket(Test):
    ...
    def _server_listen_argument(self):
        ...
        try:
            ...
            listener.close()
            raise RuntimeError(
                f"Failed to bind Unix socket at {self.unix_socket_path}: {ex}"
            )
        return listener
