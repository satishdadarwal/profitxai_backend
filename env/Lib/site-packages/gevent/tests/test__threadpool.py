...
if (
    greentest.PYPY and (greentest.WIN or greentest.RUN_COVERAGE)
) or greentest.RUN_LEAKCHECKS:
...
class TestErrorInIterator(TestCase):
    ...
    def test(self):
        ...
        self.assertRaises(
            greentest.ExpectedException, self.pool.map, lambda x: None, error_iter()
        )
        gevent.sleep(0.001)
...
@greentest.skipUnless(
    hasattr(gevent.threadpool, "ThreadPoolExecutor"), "Requires ThreadPoolExecutor"
)
