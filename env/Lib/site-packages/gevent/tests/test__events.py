...
class TestImplements(unittest.TestCase):
    ...
    def test_mem_threshold(self):
        verify.verifyClass(
            events.IMemoryUsageThresholdExceeded, events.MemoryUsageThresholdExceeded
        )
        verify.verifyObject(
            events.IMemoryUsageThresholdExceeded,
            events.MemoryUsageThresholdExceeded(0, 0, 0),
        )

    def test_mem_decreased(self):
        verify.verifyClass(
            events.IMemoryUsageUnderThreshold, events.MemoryUsageUnderThreshold
        )
        verify.verifyObject(
            events.IMemoryUsageUnderThreshold,
