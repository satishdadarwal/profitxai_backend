...
class TestAsyncResult(greentest.TestCase):
    ...
    def test_set_with_timeout(self):
        ...
        self.assertIs(
            result, X, "Nobody sent anything to event2 yet it received %r" % (result,)
        )
    ...
    def _check_pypy_switch(self):
        ...
        if greentest.PYPY:
            ...
            if sys.pypy_version_info[:3] <= (7, 3, 3):  # pylint:disable=no-member
                self.skipTest(
                    "PyPy bug: https://foss.heptapod.net/pypy/pypy/-/issues/3381"
                )
...
class TestWait(greentest.TestCase):
    ...
    @greentest.skipOnAppVeyor(
        "Not all results have arrived sometimes due to timer issues"
    )
    def test(self):
        ...
        if self.count is None:
            ...
            sender.kill()
        self.assertEqual(
            expected_len, len(results), (expected_len, len(results), results)
        )
