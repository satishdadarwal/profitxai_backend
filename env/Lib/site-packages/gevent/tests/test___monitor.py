...
class TestPeriodicMonitoringThread(
    _AbstractTestPeriodicMonitoringThread, unittest.TestCase
):
    ...
    def test_add_monitoring_function(self):
        ...
        self.assertRaises(
            ValueError, self.pmt.add_monitoring_function, lambda: None, -1
        )
        ...
        self.assertEqual(
            self.len_pmt_default_funcs + 1, len(self.pmt.monitoring_functions())
        )
        ...
        self.assertEqual(
            self.len_pmt_default_funcs, len(self.pmt.monitoring_functions())
        )

    def test_calculate_sleep_time(self):
        self.assertEqual(
            self.pmt.monitoring_functions()[0].period, self.pmt.calculate_sleep_time()
        )
        ...
        self.assertEqual(
            self.pmt.monitoring_functions()[0].period, self.pmt._calculated_sleep_time
        )
...
class TestPeriodicMonitorBlocking(
    _AbstractTestPeriodicMonitoringThread, unittest.TestCase
):
...
class TestPeriodicMonitorMemory(
    _AbstractTestPeriodicMonitoringThread, unittest.TestCase
):
