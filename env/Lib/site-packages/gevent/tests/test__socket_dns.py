...
def add(
    klass, hostname, name=None, skip=None, skip_reason=None, require_equal_errors=True
):
    ...
    def test_gethostbyname(self):
        x = hostname() if call else hostname
        ipaddr = self._test(
            "gethostbyname", x, require_equal_errors=require_equal_errors
        )
        if not isinstance(ipaddr, Exception):
            self._test(
                "gethostbyaddr", ipaddr, require_equal_errors=require_equal_errors
            )
...
class TestCase(greentest.TestCase):
    ...
    def _run_test_generic(
        self, func_name, real_func, gevent_func, func_args, require_equal_errors=True
    ):
    ...
    NORMALIZE_GAI_IGNORE_CANONICAL_NAME = (
        RESOLVER_ARES  # It tends to return them even when not asked for
    )
        ...
        def _normalize_result_getaddrinfo(self, result):
            ...
            if self.NORMALIZE_GAI_IGNORE_CANONICAL_NAME:
                result = [
                    ...
                    for family, kind, proto, _, addr in result
                ]
    ...
    def _compare_exceptions_lenient(self, real_result, gevent_result, func_name):
        try:
            ...
            if (
                ...
                not in (socket.herror, socket.gaierror, socket.error)
            ):
                raise
            util.log(
    ...
    _compare_exceptions = (
        _compare_exceptions_lenient
        if RESOLVER_NOT_SYSTEM
        else _compare_exceptions_strict
    )
    ...
    def _compare_results(self, real_result, gevent_result, func_name):
        ...
        compare_func = getattr(
            self, "_compare_results_" + func_name, self._generic_compare_results
        )
    ...
    def _generic_compare_results(self, real_result, gevent_result, func_name):
        ...
        return all(
            self._compare_results(x, y, func_name)
            for (x, y) in zip(real_result, gevent_result)
        )
    ...
    def _compare_results_getaddrinfo(self, real_result, gevent_result, func_name):
        ...
        # we'll take it.
        errors = isinstance(real_result, Exception) + isinstance(
            gevent_result, Exception
        )
        if errors == 2:
            return True
        if errors == 1:
    ...
    def assertEqualResults(
        self, real_result, gevent_result, func_name, require_equal_errors=True
    ):
        errors = (
            OverflowError,
            TypeError,
...
add(
    ...
    skip_reason=(
        "Sometimes get a different result for getaddrinfo "
...
class SanitizedHostsFile(HostsFile):
    def iter_all_host_addr_pairs(self):
        for name, addr in super(SanitizedHostsFile, self).iter_all_host_addr_pairs():
            if RESOLVER_NOT_SYSTEM and (
                name.endswith("local")  # ignore bonjour, ares can't find them
...
class TestEtcHosts(TestCase):
    ...
    def populate_tests(cls):
        hf = SanitizedHostsFile(
            os.path.join(os.path.dirname(__file__), "hosts_file.txt")
        )
...
class Test_getaddrinfo(TestCase):
    ...
    def test1(self):
        return self._test_getaddrinfo(
            TestGeventOrg.HOSTNAME, 52, socket.AF_UNSPEC, socket.SOCK_STREAM, 0, 0
        )

    def test2(self):
        return self._test_getaddrinfo(
            TestGeventOrg.HOSTNAME, 53, socket.AF_INET, socket.SOCK_DGRAM, 17
        )

    @unittest.skipIf(
        RESOLVER_DNSPYTHON, "dnspython only returns some of the possibilities"
    )
    def test3(self):
    ...
    def test_enums(self):
        ...
        gai = gevent_socket.getaddrinfo(
            "example.com", 80, socket.AF_INET, socket.SOCK_STREAM, socket.IPPROTO_TCP
        )
...
add(TestInternational, "президент.рф", "russian", require_equal_errors=False)
add(
    TestInternational, "президент.рф".encode("idna"), "idna", require_equal_errors=False
)
