...
class TestOS_nb(TestOS_tp):
    ...
    def _test_event_closed_on_normal_io(
        self, nb_func, nb_arg, mock_io, mock_get_hub, event
    ):
    ...
    def _test_event_closed_on_io_error(
        self, nb_func, nb_arg, mock_io, mock_get_hub, event
    ):
    ...
    def test_event_closed_on_write(self, mock_write, mock_get_hub):
        self._test_event_closed_on_normal_io(
            os.nb_write, b"buf", mock_write, mock_get_hub, 2
        )

    @mock.patch("gevent.os.get_hub")
    @mock.patch("gevent.os._write")
    def test_event_closed_on_write_error(self, mock_write, mock_get_hub):
        self._test_event_closed_on_io_error(
            os.nb_write, b"buf", mock_write, mock_get_hub, 2
        )
    ...
    def test_event_closed_on_read(self, mock_read, mock_get_hub):
        self._test_event_closed_on_normal_io(
            os.nb_read, b"buf", mock_read, mock_get_hub, 1
        )
