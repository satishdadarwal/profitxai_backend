...
def print_run_info(thread_stacks=True, greenlet_stacks=True, limit=_NONE, file=None):
    ...
    lines = format_run_info(
        thread_stacks=thread_stacks, greenlet_stacks=greenlet_stacks, limit=limit
    )
...
def format_run_info(
    thread_stacks=True, greenlet_stacks=True, limit=_NONE, current_thread_ident=None
):
    """
        ...
        lines.extend(
            tree.format_lines(
                details={
                    ...
                    "running_stacks": (
                        greenlet_stacks if tree.is_current_tree else False
                    ),
...
class _TreeFormatter(object):
    ...
    def child_tail(self, line, vertical=VERTICAL):
        return " " * self.indent + vertical + " " * self.horiz_width + line
...
class GreenletTree(object):
    ...
    def format_lines(self, details=True):
        ...
        lines = [l[0] if isinstance(l, tuple) else l for l in self._render(tree)]
        return lines
    ...
    def _render(self, tree):
        ...
        if getattr(self.greenlet, "gevent_monitoring_thread", None) is not None:
            tree.child_data(
                "Monitoring Thread:" + repr(self.greenlet.gevent_monitoring_thread())
            )
        ...
        if tree_locals and tree_locals is not getattr(
            spawning_parent, "spawn_tree_locals", None
        ):
    ...
    def _root_greenlet(greenlet):
        while greenlet.parent is not None and not getattr(
            greenlet, "greenlet_tree_is_root", False
        ):
    ...
    def _forest(cls):
        ...
        for ob in glets:
            ...
            if (
                spawn_parent is root_greenlet(spawn_parent)
                and spawn_parent not in roots
            ):
