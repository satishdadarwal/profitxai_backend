...
__imports__ = copy_globals(
    __socket__, globals(), only_names=__imports__, ignore_missing_names=True
)
...
def _sendall(
    ...
    SO_SNDBUF=__socket__.SO_SNDBUF,
        ...
        with Timeout._start_new_or_dummy(self.timeout, __socket__.timeout("timed out")):
            while 1:
                ...
                if not result or result == EISCONN:
                    break
                if (result in (EWOULDBLOCK, EINPROGRESS, EALREADY)) or (
                    result == EINVAL and is_windows
                ):
