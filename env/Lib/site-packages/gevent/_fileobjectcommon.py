...
class UniversalNewlineBytesWrapper(io.TextIOWrapper):
    ...
    def __init__(self, fobj, line_buffering):
        ...
        io.TextIOWrapper.__init__(
            self, fobj, encoding="latin-1", newline=None, line_buffering=line_buffering
        )
...
class OpenDescriptor(object):  # pylint:disable=too-many-instance-attributes
    ...
    def _collapse_arg(pref_name, preferred_val, old_name, old_val, default):
        ...
        if preferred_val is not None and old_val is not None:
            raise TypeError(
                "Cannot specify both %s=%s and %s=%s"
                % (pref_name, preferred_val, old_name, old_val)
            )
    ...
    def __init__(
        ...
        self.native = (
            not self.text
            and not self.binary  # Neither t nor b given.
            and not encoding
            and not errors  # And no encoding or error handling either.
        )
        ...
        if result is not raw or self._raw_object_is_new(raw):
            if result.__class__ is FileIO:
                ...
                raise NotImplementedError(
                    ...
                    "Please open a gevent issue with your use-case." % (result)
                )
        return result
