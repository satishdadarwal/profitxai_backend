...
def get_header_version():
    return "libuv-%d.%d.%d" % (
        libuv.UV_VERSION_MAJOR,
        libuv.UV_VERSION_MINOR,
        libuv.UV_VERSION_PATCH,
    )
...
class loop(AbstractLoop):
    ...
    _TIMER_POINTER = (
        _CHECK_POINTER  # This is poorly named. It's for the callback "timer"
    )
    ...
    def debug(self):
        ...
        libuv.uv_walk(
            self.ptr, ffi.callback("void(*)(uv_handle_t*,void*)", walk), ffi.NULL
        )
        return handles
    ...
    def run(self, nowait=False, once=False):
        ...
        if mode == libuv.UV_RUN_DEFAULT:
            while self._ptr and self._ptr.data:
                ...
                if (
                    not ran_status
                    and not ran_callbacks
                    and not self._prepare_ran_callbacks
                ):
    ...
    def install_sigchld(self):
        ...
        libuv.uv_signal_start(
            self._sigchld_watcher, libuv.python_sigchld_callback, signal.SIGCHLD
        )
    ...
    def _sigchld_callback(self):
        ...
        while True:
            ...
            if pid == 0:
                break
            children_watchers = self._child_watchers.get(
                pid, []
            ) + self._child_watchers.get(0, [])
    ...
    def io(self, fd, events, ref=True, priority=None):
        ...
        try:
            ...
            assert io_watcher._multiplex_watchers, (
                "IO Watcher %s unclosed but should be dead" % io_watcher
            )
        except KeyError:
    ...
    def prepare(self, ref=True, priority=None):
        ...
        raise TypeError(
            "prepare watchers are not currently supported in libuv. "
