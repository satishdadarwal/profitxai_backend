...
class SSLContext(orig_SSLContext):
    ...
    if hasattr(orig_SSLContext, "_msg_callback"):
        ...
        def _msg_callback(self, value):
            ...
            try:
                super(SSLContext, SSLContext)._msg_callback.__set__(
                    self, value
                )  # pylint:disable=no-member
            finally:
    ...
    if hasattr(orig_SSLContext, "sni_callback"):
        ...
        def sni_callback(self, value):
            if value and callable(value):
                value = _Callback(value)
            super(orig_SSLContext, orig_SSLContext).sni_callback.__set__(
                self, value
            )  # pylint:disable=no-member
...
def get_server_certificate(addr, ssl_version=PROTOCOL_SSLv23, ca_certs=None):
    ...
    with create_connection(addr) as sock:
        with wrap_socket(
            sock, ssl_version=ssl_version, cert_reqs=cert_reqs, ca_certs=ca_certs
        ) as sslsock:
