...
def notify_and_call_entry_points(event):
    ...
    if sys.version_info[:2] >= (3, 10):
        ...
        entry_points = set(ep for ep in ep_dict.get(event.ENTRY_POINT_NAME, ()))
...
class IEventLoopBlocked(Interface):
    ...
    blocking_time = Attribute(
        "The approximate time in seconds the loop has been blocked."
    )
    info = Attribute(
        "A list of string lines providing extra info. You may modify this list."
    )
...
class MemoryUsageUnderThreshold(_AbstractMemoryEvent):
    ...
    def __init__(self, mem_usage, max_allowed, memory_info, max_usage):
        super(MemoryUsageUnderThreshold, self).__init__(
            mem_usage, max_allowed, memory_info
        )
...
class GeventPatchEvent(object):
    ...
    def __repr__(self):
        return "<%s source=%r target=%r at %x>" % (
            self.__class__.__name__,
            self.source,
            self.target,
            id(self),
