...
class local(object):
    ...
    def __cinit__(self, *args, **kw):  # pylint:disable=bad-dunder-name
        if args or kw:
            if (
                type(self).__init__ == object.__init__
    ...
    def __getattribute__(self, name):  # pylint:disable=too-many-return-statements
        ...
        if name in self._local_type_vars:
            ...
            for base in self._local_type.mro():
                ...
                if name in bd:
                    ...
                    result = type(attr_on_type).__get__(
                        attr_on_type, self, self._local_type
                    )
                    return result
        ...
        raise AttributeError(
            "%r object has no attribute '%s'" % (self._local_type.__name__, name)
        )

    def __setattr__(self, name, value):
        if name == "__dict__":
            raise AttributeError(
                "%r object attribute '__dict__' is read-only" % type(self)
            )
