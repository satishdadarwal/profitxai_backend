...
def _resolve_special(hostname, family):
    if not isinstance(hostname, hostname_types):
        raise TypeError(
            "argument 1 must be str, bytes or bytearray, not %s" % (type(hostname),)
        )
...
class AbstractResolver(object):
    ...
    EAI_NONAME_MSG = (
        ...
        if MAC
        else "Name or service not known"
    )

    EAI_FAMILY_MSG = "ai_family not supported"

    _KNOWN_ADDR_FAMILIES = {v for k, v in vars(_socket).items() if k.startswith("AF_")}

    _KNOWN_SOCKTYPES = {
        v
        for k, v in vars(_socket).items()
        if k.startswith("SOCK_") and k not in ("SOCK_CLOEXEC", "SOCK_MAX_SIZE")
    }
    ...
    def _hostname_to_bytes(self, hostname):
        if isinstance(hostname, text_type):
            ...
            raise TypeError(
                "Expected str, bytes or bytearray, not %s" % type(hostname).__name__
            )
    ...
    def _gethostbyname_ex(self, hostname_bytes, family):
        ...
        addresses = []
        tuples = self.getaddrinfo(
            hostname_bytes, 0, family, SOCK_STREAM, SOL_TCP, AI_CANONNAME
        )
