...
class Resolver(AbstractResolver):
    ...
    def __repr__(self):
        return "<gevent.resolver_ares.Resolver at 0x%x ares=%r>" % (
            id(self),
            self.cares,
        )
    ...
    def __getaddrinfo(
        ...
        if fill_in_type_proto:
            ...
            result = [
                ...
                for rfamily, rtype, rproto, rcanon, raddr in result
                for hard_type, hard_proto in hard_type_proto
            ]
        return result
    ...
    def _getaddrinfo(self, host_bytes, port, family, socktype, proto, flags):
        while True:
            ares = self.cares
            try:
                return self.__getaddrinfo(
                    host_bytes, port, family, socktype, proto, flags
                )
            except gaierror:
                if ares is self.cares:
                    raise
