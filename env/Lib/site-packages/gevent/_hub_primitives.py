...
class WaitOperationsGreenlet(
    ...
    def cancel_waits_close_and_then(self, watchers, exc_kind, then, *then_args):
        ...
        if deferred:
            self.loop.run_callback(
                self._cancel_waits_then, deferred, exc_kind, then, then_args
            )
        else:
            then(*then_args)
...
def wait_on_socket(socket, watcher, timeout_exc=None):
    if socket is None or watcher is None:
        ...
        raise ConcurrentObjectUseError(
            "The socket has already been closed by another greenlet"
        )
    _primitive_wait(
        watcher,
        socket.timeout,
