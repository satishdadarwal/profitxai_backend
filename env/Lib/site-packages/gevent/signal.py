...
def signal(signalnum, handler):
    ...
    if (
        handler != _signal.SIG_IGN
        and handler != _signal.SIG_DFL
...
def _write_child_signal_fd(fd, warn_on_full_buffer):
    ...
    try:
        ...
        if (
            warn_on_full_buffer or e.errno not in gevent.os.ignored_errors
...
if "waitpid" in gevent.os.__implements__ and hasattr(
    _signal, "SIGCHLD"
):  # pylint: disable=no-member
