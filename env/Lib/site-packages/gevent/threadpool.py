...
class _WorkerGreenlet(RawGreenlet):
    ...
    def __print_tb(tb, stderr):
        ...
        while tb is not None:
            ...
            name = co.co_name
            print('  File "%s", line %d, in %s' % (filename, lineno, name), file=stderr)
            tb = tb.tb_next

    def _before_run_task(
        self,
        func,
        args,
        kwargs,
    ...
    def run(self):
        ...
        try:  # pylint:disable=too-many-nested-blocks
            ...
            print(
                "Failed to run worker thread. Task=%r Exception=%r" % (task, e),
                file=self._stderr,
            )
...
class ThreadPool(GroupMappingMixin):
    ...
    def _adjust_step(self):
        ...
        while len(
            self._worker_greenlets
        ) < self._maxsize and self.task_queue.unfinished_tasks > len(
            self._worker_greenlets
        ):
            self._add_thread()
        ...
        while (
            len(self._worker_greenlets) - self._maxsize
