...
class SubscriptionInfo:
    ...
    def subscribe(
        self, symbols: Set[str], channelNo: str, mode: SubscriptionModes
    ) -> None:
...
class DataStore:
    ...
    def updateDepth(
        self, packet: protomsg.SocketMessage, cb: Optional[Callable], diffOnly: bool
    ):
...
class FyersTbtSocket:
    ...
    def subscribe(
        self, symbol_tickers: Set[str], channelNo: str, mode: SubscriptionModes
    ) -> None:
        """
    ...
    def switchChannel(
        self, resume_channels: Set[str], pause_channels: Set[str]
    ) -> None:
        """
        ...
        try:
            if self.onopen:
                ...
                for channel in open_chans:
                    self.subscribe(
                        self._subsinfo.getSymbolsInfo(channel),
                        channel,
                        self._subsinfo.getModeInfo(channel),
                    )
        except Exception as e:
            self.On_error(e)
