            ...
            if response_data["s"] == "ok":
                for symbol, fytoken in response_data["validSymbol"].items():
                    ...
                    if (
                        ...
                        self.data_type == "DepthUpdate" and symbol_split[-1] != "INDEX"
                    ):
...
class FyersDataSocket:
    ...
    def __new__(cls, *args, **kwargs):
        # PATCHED: singleton disabled — fresh object on each instantiation
            ...
            for _ in range(scrip_count):
                ...
                if data_type == 83:  # Snapshot datafeed
                    ...
                    if topic_id in self.scrips_sym:
                        ...
                        if (
                            value
                            != self.resp[self.scrips_sym[topic_id]][self.data_val[0]]
                            and value != -2147483648
                        ):
                            self.resp[self.scrips_sym[topic_id]][
                                self.data_val[0]
                        ...
                        if (
                            value
                            != self.resp[self.index_sym[topic_id]][self.index_val[0]]
                            and value != -2147483648
                        ):
                            self.resp[self.index_sym[topic_id]][
                                self.index_val[0]
    ...
    def __process_message_queue(self) -> None:
        ...
        while not self.message_thread_stop_event.is_set():
            with self.message_lock:
                while (
                    not self.message_thread_stop_event.is_set() and not self.message
    ...
    def __ping(self):
        while (
            ...
            self.__ws_object.send(
                bytes([0, 1, 11]), opcode=websocket.ABNF.OPCODE_BINARY
            )
            time.sleep(5)
