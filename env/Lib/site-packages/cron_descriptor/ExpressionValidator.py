...
class ExpressionValidator:
    ...
    def _validate_expression(
        self, expression_parts: list[str], expr_length: int
    ) -> None:
        ...
        */nn
        ...
        if re.match(r"\d{1,2}$", expr):
            ...
            if expr == "*":
                ...
                self.check_range(expr=parts[0], mi=mi, mx=mx, prefix=prefix)
                self.check_range(
                    type_="interval", expr=parts[1], mi=mi, mx=mx, prefix=prefix
                )
                ...
                r"^(\d{1,2}|\d{1,2}-\d{1,2})(,\d{1,2}|,\d{1,2}-\d{1,2})+$", expr
            ):
                limit = 60
                expr_ls = expr.split(",")
        ...
        */nn
        ...
        if re.match(r"\d{1,2}$", expr):
            ...
            if expr in ("*", "?"):
                ...
                self.check_range(expr=parts[0], mi=mi, mx=mx, prefix=prefix)
                self.check_range(
                    type_="interval", expr=parts[1], mi=0, mx=mx, prefix=prefix
                )
        ...
        */nn
        ...
        if re.match(r"\d{1,2}$", expr):
            ...
            if expr == "*":
                ...
                self.check_range(
                    type_="interval", expr=parts[1], mi=0, mx=12, prefix=prefix
                )
                ...
                r"^((\d{1,2}|\D{3})|(\D{3}-\D{3})|(\d{1,2}-\d{1,2}))((,\d{1,2})+"
                r"|(,\D{3})*|(,\d{1,2}-\d{1,2})*|(,\D{3}-\D{3})*)*$",
                expr,
            ):
                """
                ...
                for month in expr_ls:
                    if "-" in month:
                        ...
                        if len(parts[0]) == 3:
                            self.check_range(
                                expr=cron_months[parts[0].upper()],
                                mi=mi,
                                mx=mx,
                                prefix=prefix,
                            )
                            self.check_range(
                                expr=cron_months[parts[1].upper()],
                                mi=mi,
                                mx=mx,
                                prefix=prefix,
                            )
                        ...
                        cron_month = (
                            cron_months[month.upper()] if len(month) == 3 else month
                        )
    ...
    def dayofweek(self, expr: str, prefix: str) -> None:
        ...
        */n
        ...
        if expr in ("*", "?"):
            ...
            self.compare_range(
                st=st_day, ed=ed_day, mi=mi, mx=mx, prefix=prefix, type_="dow"
            )

        elif re.match(
            r"^((\d{1}|\D{3})|(\D{3}-\D{3})|(\d{1}-\d{1}))"
            r"((,\d{1})+|(,\D{3})*|(,\d{1}-\d{1})*|(,\D{3}-\D{3})*)*$",
            expr,
        ):
            limit = 7
            expr_ls = expr.split(",")
            ...
            for day in expr_ls:
                if "-" in day:
                    ...
                    if len(parts[0]) == 3:
                        self.check_range(
                            expr=cron_days[parts[0].upper()],
                            mi=mi,
                            mx=mx,
                            prefix=prefix,
                        )
                        self.check_range(
                            expr=cron_days[parts[1].upper()],
                            mi=mi,
                            mx=mx,
                            prefix=prefix,
                        )
        ...
        */nnn(0~129)
        ...
        if re.match(r"\d{4}$", expr):
            ...
            if expr == "*":
                ...
                self.check_range(expr=parts[0], mi=mi, mx=mx, prefix=prefix)
                self.check_range(
                    type_="interval", expr=parts[1], mi=0, mx=129, prefix=prefix
                )
    ...
    def check_range(
        self, prefix: str, mi: int, mx: int, expr: str | int, type_: str | None = None
    ) -> None:
        """
